...
public:
    ...
    [[nodiscard]] long long get_scheduled_time() const {
...
private:
    ...
    void run_loop() {
        ...
        debug("SleepScheduler run_loop exit.");
    }
...
public:
    ...
    SleepScheduler() {
        ...
        work_thread = std::thread(&SleepScheduler::run_loop, this);
    }
    ...
    void execute(std::function<void()> &&func, long long delay) {
        ...
        if (is_active.load(std::memory_order_relaxed)) {
            ...
            executable_queue.emplace(std::move(func), delay);
            lock.unlock();
            if (need_notify) {
